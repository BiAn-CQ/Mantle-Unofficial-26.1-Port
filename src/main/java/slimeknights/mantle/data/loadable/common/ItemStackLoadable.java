...
public class ItemStackLoadable {
  ...
  /** Field for item stack count that allows empty */
  private static final LoadableField<CompoundTag,ItemStack> NBT = NBTLoadable.ALLOW_STRING.nullableField("nbt", stack -> stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag());
  ...
  private enum NBTStack implements RecordLoadable<ItemStack> {
    ...
    public void encode(FriendlyByteBuf buffer, ItemStack stack) throws EncoderException {
      ...
      buffer.writeNbt(stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag());
    }
  }
}
