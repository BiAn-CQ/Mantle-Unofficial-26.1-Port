...
public class RegistryDataMapLoader<R,D> extends SimpleJsonResourceReloadListener<Dynamic<?>> {
  ...
  @Override
  protected void apply(Map<Identifier,Dynamic<?>> jsons, ResourceManager resourceManager, ProfilerFiller profiler) {
    long time = System.nanoTime();
    Map<Identifier,JsonElement> jsonElements = new HashMap<>();
    for (Entry<Identifier,Dynamic<?>> entry : jsons.entrySet()) {
      jsonElements.put(entry.getKey(), entry.getValue().convert(JsonOps.INSTANCE).getValue());
    }
    ...
    for (Entry<ResourceKey<R>,R> entry : registry.entrySet()) {
      ...
      Dynamic<?> dynamic = jsons.get(location);
      if (dynamic != null) {
        ...
        try {
          ...
          dataMap.put(entry.getValue(), parseData(name, jsonElements, location, json, locationMap, dataLoader, context, merger));
        } catch (Exception e) {
