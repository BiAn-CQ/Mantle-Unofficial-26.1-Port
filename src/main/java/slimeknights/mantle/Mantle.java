...
import slimeknights.mantle.recipe.ingredient.MantleIngredientTypes;
...
public class Mantle {
  ...
  public Mantle(IEventBus bus, ModContainer modContainer) {
    ...
    bus.addListener(EventPriority.NORMAL, false, GatherDataEvent.Server.class, this::gatherServerData);
    bus.addListener(EventPriority.NORMAL, false, GatherDataEvent.Client.class, this::gatherClientData);
    bus.addListener(EventPriority.NORMAL, false, RegisterEvent.class, this::register);
    bus.addListener(EventPriority.NORMAL, false, RegisterPayloadHandlersEvent.class, NetworkWrapper::registerPayloads);
  ...
  private void registerCapabilities(RegisterCapabilitiesEvent event) {
    ...
    BuiltInRegistries.ITEM.stream()
      .filter(FluidContainerFoodItem.class::isInstance)
      .map(FluidContainerFoodItem.class::cast)
      .forEach(item -> event.registerItem(Capabilities.Fluid.ITEM, (stack, access) -> item.createFluidHandler(access), item));
    Block[] blockEntityBlocks = BuiltInRegistries.BLOCK_ENTITY_TYPE.stream()
      .flatMap(type -> type.getValidBlocks().stream())
      .distinct()
      .toArray(Block[]::new);
  ...
  @SuppressWarnings("removal") // Initializes public compatibility aliases retained for dependent 26.1 mods.
  private void commonSetup(final FMLCommonSetupEvent event) {
    MantleRecipeSerializers.bootstrap();
    MantleCommand.init();
  ...
  private void register(RegisterEvent event) {
    ...
    if (key == NeoForgeRegistries.Keys.CONDITION_CODECS) {
      event.register(NeoForgeRegistries.Keys.CONDITION_CODECS, getResource("tag_empty"), () -> TagEmptyCondition.CODEC);
      event.register(NeoForgeRegistries.Keys.CONDITION_CODECS, getResource("tag_filled"), () -> TagFilledCondition.CODEC);
      ...
      // predicates
      {
        ...
        // register mob types
        TagKey<EntityType<?>> undefined = TagKey.create(Registries.ENTITY_TYPE, getResource("undefined_mob_type"));
        MobTypePredicate.MOB_TYPES.register(Identifier.parse("undefined"), undefined);
        MobTypePredicate.MOB_TYPES.register(Identifier.parse("undead"), EntityTypeTags.UNDEAD);
        MobTypePredicate.MOB_TYPES.register(Identifier.parse("arthropod"), EntityTypeTags.ARTHROPOD);
        MobTypePredicate.MOB_TYPES.register(Identifier.parse("illager"), EntityTypeTags.ILLAGER);
        MobTypePredicate.MOB_TYPES.register(Identifier.parse("water"), EntityTypeTags.AQUATIC);
      ...
      BlockEntityTypeRegistryAdapter adapter = new BlockEntityTypeRegistryAdapter(
        Objects.requireNonNull(event.getRegistry(Registries.BLOCK_ENTITY_TYPE)));
      ...
      event.register(Registries.COMMAND_ARGUMENT_TYPE, getResource("resource_or_tag_key"), () -> {
        ResourceOrTagKeyArgument.Info<?> info = new ResourceOrTagKeyArgument.Info<>();
        return ArgumentTypeInfos.registerByClass(RegistrationHelper.genericArgumentType(ResourceOrTagKeyArgument.class), info);
      });
    }
    else {
  ...
  private void gatherServerData(final GatherDataEvent.Server event) {
    ...
    event.addProvider(new MantleBlockTagProvider(packOutput, lookupProvider));
    event.addProvider(new MantleFluidTagProvider(packOutput, lookupProvider));
    event.addProvider(new MantleMenuTagProvider(packOutput, lookupProvider));
    event.addProvider(new MantleFluidTransferProvider(packOutput));
  }

  private void gatherClientData(final GatherDataEvent.Client event) {
    event.addProvider(new MantleFluidTooltipProvider(event.getGenerator().getPackOutput()));
  }
  ...
  public static MutableComponent makeComponent(String base, String name, Object... args) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (arg instanceof Component || arg instanceof Number || arg instanceof Boolean || arg instanceof String) {
        ...
        safeArgs[i] = String.valueOf(arg);
      }
    }
    return Component.translatable(makeDescriptionId(base, name), safeArgs);
  }
}
