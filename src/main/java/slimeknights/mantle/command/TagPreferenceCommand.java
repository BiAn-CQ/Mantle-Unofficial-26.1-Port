...
public class TagPreferenceCommand {
  ...
  private static <T> int runGeneric(CommandContext<CommandSourceStack> context, Registry<T> registry) {
    ...
    if (preference == null) {
      context.getSource().sendSuccess(() -> Component.translatable(EMPTY_TAG, registry.key().identifier().toString(), String.valueOf(name)), true);
      return 0;
    } else {
      context.getSource().sendSuccess(() -> Component.translatable(PREFERENCE, registry.key().identifier().toString(), String.valueOf(name), String.valueOf(registry.getKey(preference))), true);
      return 1;
    }
  }
}
