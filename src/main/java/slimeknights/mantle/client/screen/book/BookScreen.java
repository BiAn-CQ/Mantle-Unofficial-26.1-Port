...
public class BookScreen extends Screen {
  ...
  public static Font getAltFont() {
    if (altFont == null) {
      ...
      altFont = new Font(new Font.Provider() {
        @Override
        public GlyphSource glyphs(net.minecraft.network.chat.FontDescription font) {
          return resourceManager.fontSets.get(Minecraft.ALT_FONT).source(false);
        }

        @Override
        public EffectGlyph effect() {
          return resourceManager.fontSets.get(Minecraft.ALT_FONT).whiteGlyph();
        }
      });
    }
    return altFont;
  ...
  public static Font getUniformFont() {
    if (uniformFont == null) {
      FontManager resourceManager = Minecraft.getInstance().fontManager;
      uniformFont = new Font(new Font.Provider() {
        @Override
        public GlyphSource glyphs(net.minecraft.network.chat.FontDescription font) {
          return resourceManager.fontSets.get(Minecraft.UNIFORM_FONT).source(false);
        }

        @Override
        public EffectGlyph effect() {
          return resourceManager.fontSets.get(Minecraft.UNIFORM_FONT).whiteGlyph();
        }
      });
    }
    return uniformFont;
  ...
  public Font getFontRenderer() {
    ...
    if (fontRenderer == null) {
      ...
      // Page content builders use BookData.fontRenderer directly.  Persist
      // the fallback so rendering and page construction cannot disagree.
      this.book.fontRenderer = fontRenderer;
    }
...
//      rightMatrix.scale(PAGE_SCALE, PAGE_SCALE, 1F);
  ...
  private void renderCover(GuiGraphicsExtractor graphics, Vector3f coverColor) {
    ...
    int coverTint = ARGB.color(255, (int)(coverColor.x() * 255), (int)(coverColor.y() * 255), (int)(coverColor.z() * 255));
    graphics.blit(RenderPipelines.GUI_TEXTURED, cover, centerX, centerY, 0, 0, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE, coverTint);

    Matrix3x2fStack matrixStack = graphics.pose();
    if (!this.book.appearance.title.isEmpty()) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, cover, centerX, centerY, 0, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE, -1);
  ...
  private void renderUnderLayer(GuiGraphicsExtractor graphics, Vector3f coverColor) {
    int tint = ARGB.color(255, (int)(coverColor.x() * 255), (int)(coverColor.y() * 255), (int)(coverColor.z() * 255));
    graphics.blit(RenderPipelines.GUI_TEXTURED, this.book.appearance.getBookTexture(), this.width / 2 - PAGE_WIDTH_UNSCALED, this.height / 2 - PAGE_HEIGHT_UNSCALED / 2, 0, 0, PAGE_WIDTH_UNSCALED * 2, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED * 2, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE, tint);
  }
  ...
  private void renderPageBackground(GuiGraphicsExtractor graphics, boolean rightSide) {
    ...
    int pageTintColor = ARGB.color(255, (int)(pageTint.x() * 255), (int)(pageTint.y() * 255), (int)(pageTint.z() * 255));
    if(!rightSide) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, book.appearance.getBookTexture(), this.width / 2 - PAGE_WIDTH_UNSCALED, this.height / 2 - PAGE_HEIGHT_UNSCALED / 2, 0, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE, pageTintColor);
    } else {
      graphics.blit(RenderPipelines.GUI_TEXTURED, book.appearance.getBookTexture(), this.width / 2, this.height / 2 - PAGE_HEIGHT_UNSCALED / 2, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, PAGE_WIDTH_UNSCALED, PAGE_HEIGHT_UNSCALED, TEX_SIZE, TEX_SIZE, pageTintColor);
    }
  }
