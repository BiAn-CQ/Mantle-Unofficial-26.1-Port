...
public class TextComponentDataRenderer {
  ...
  public static String drawText(GuiGraphicsExtractor graphics, int x, int y, int boxWidth, int boxHeight, TextComponentData[] data, int mouseX, int mouseY, Font fr, List<Component> tooltip) {
    ...
    for (TextComponentData item : data) {
      ...
      if (atY >= y + boxHeight) {
        graphics.text(fr, "...", atX, atY, opaqueColor(0), item.dropShadow);
        break;
      }
  ...
  public static void drawScaledTextComponent(GuiGraphicsExtractor graphics, Font font, FormattedText textComponent, float x, float y, boolean dropShadow, float scale) {
    ...
    graphics.text(font, Language.getInstance().getVisualOrder(textComponent), 0, 0, opaqueColor(0), dropShadow);
    poseStack.popMatrix();
  }

  private static int opaqueColor(int color) {
    return (color & 0xFF000000) != 0 ? color : color | 0xFF000000;
  }
}
