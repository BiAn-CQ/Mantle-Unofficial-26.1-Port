...
public class ScalableElementScreen extends ElementScreen {
  ...
  public int drawScaledX(GuiGraphicsExtractor graphics, int xPos, int yPos, int width) {
    ...
    if (remainder > 0) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, texture, xPos + width - remainder, yPos, this.x, this.y, remainder, this.h, remainder, this.h, this.texW, this.texH, -1);
    }
  ...
  public int drawScaledY(GuiGraphicsExtractor graphics, int xPos, int yPos, int height) {
    ...
    if (remainder > 0) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, texture, xPos, yPos + height - remainder, this.x, this.y, this.w, remainder, this.w, remainder, this.texW, this.texH, -1);
    }
  ...
  public int drawScaledYUp(GuiGraphicsExtractor graphics, int xPos, int yPos, int height) {
    ...
    int remainder = height % this.h;
    // Draw complete tiles from the bottom upwards. The old implementation
    // only drew the remainder, so a full-height fire was invisible.
    for (int i = 0; i < full; i++) {
      this.draw(graphics, xPos, yPos + height - (i + 1) * this.h);
    }
    if (remainder > 0) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, texture, xPos, yPos + height - full * this.h - remainder,
        this.x, this.y, this.w, remainder, this.w, remainder, this.texW, this.texH, -1);
    }
  ...
  public int drawScaled(GuiGraphicsExtractor graphics, int xPos, int yPos, int width, int height) {
    ...
    if (remainder > 0) {
      graphics.blit(RenderPipelines.GUI_TEXTURED, texture, xPos + width - remainder, yPos, this.x, this.y, remainder, yRest, remainder, yRest, this.texW, this.texH, -1);
    }
