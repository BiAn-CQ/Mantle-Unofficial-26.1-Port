...
public class BookLoader implements ResourceManagerReloadListener {
  ...
  /**
   * Minecraft 26.1 exposes chat components as an interface, so Gson cannot
   * construct them using reflection.  Book JSON still uses the native chat
   * component format, therefore delegate deserialization to Minecraft's codec.
   */
  private static final JsonDeserializer<Component> COMPONENT_DESERIALIZER = new JsonDeserializer<>() {
    @Override
    public Component deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      return ComponentSerialization.CODEC.parse(JsonOps.INSTANCE, json)
        .result()
        .orElseThrow(() -> new JsonParseException("Failed to deserialize chat component: " + json));
    }
  };
  ...
  public BookLoader() {
    ...
    registerGsonTypeAdapter(Component.class, COMPONENT_DESERIALIZER);
