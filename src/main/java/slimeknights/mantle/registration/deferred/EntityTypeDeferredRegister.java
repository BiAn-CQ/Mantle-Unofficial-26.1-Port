...
public class EntityTypeDeferredRegister extends DeferredRegisterWrapper<EntityType<?>> {
  ...
  public EntityTypeDeferredRegister(String modID) {
    super(DeferredRegister.createEntities(modID), modID);
  ...
  public <T extends Mob> EntityObject<T> registerWithEgg(String name, Supplier<EntityType.Builder<T>> sup, int primary, int secondary) {
    ...
    return new EntityObject<>(object, itemRegistry.register(name + "_spawn_egg",
      ItemDeferredRegister.withCurrentItemId(modID, name + "_spawn_egg",
        () -> new SpawnEggItem(ItemDeferredRegister.bindItemProperties(new Item.Properties().spawnEgg(object.get()))))));
  }
}
