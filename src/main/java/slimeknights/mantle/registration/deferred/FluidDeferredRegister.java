...
public class FluidDeferredRegister extends DeferredRegisterWrapper<Fluid> {
  ...
  public class Builder extends FluidBuilder<Builder> {
    ...
    public Builder commonTag(String tag) {
      this.commonTag = tag;
      return this;
    }
    ...
    /** Registers a custom inverted fluid type supplied by the caller. */
    public Builder invertedType(Supplier<? extends FluidType> type) {
      return type(type);
    }
    ...
    public Builder bucket(Function<Supplier<? extends Fluid>, Item> constructor) {
      ...
      return bucket(itemRegister.register(name + "_bucket", ItemDeferredRegister.withCurrentItemId(modID, name + "_bucket", () -> constructor.apply(stillDelayed))));
    }

    /** Creates the default bucket */
    public Builder bucket() {
      return bucket(itemRegister.register(name + "_bucket", ItemDeferredRegister.withCurrentItemId(modID, name + "_bucket", () -> new BucketItem(stillDelayed.get(), ItemDeferredRegister.bindItemProperties(RegistrationHelper.BUCKET_PROPS)))));
    }
    ...
    public Builder block(Function<Supplier<? extends FlowingFluid>, LiquidBlock> constructor) {
      ...
      return block(blockRegister.register(name + "_fluid", BlockDeferredRegister.withCurrentBlockId(modID, name + "_fluid", () -> constructor.apply((Supplier<FlowingFluid>)(Supplier)stillDelayed))));
    }
  ...
  public static BlockBehaviour.Properties createProperties(MapColor color, int lightLevel) {
    return BlockDeferredRegister.bindBlockProperties(BlockBehaviour.Properties.of().mapColor(color).replaceable().noCollision().randomTicks().strength(100.0F).lightLevel(state -> lightLevel).pushReaction(PushReaction.DESTROY).noLootTable().liquid().sound(SoundType.EMPTY));
  }
}
