...
public class MantleEvents {
  ...
  static void onLivingDeath(LivingDeathEvent event) {
    ...
    if (entity.level() instanceof ServerLevel level && !level.getGameRules().get(GameRules.KEEP_INVENTORY)
        && entity instanceof Player player && !(player instanceof FakePlayer)) {
  ...
  static void onPlayerDropItems(LivingDropsEvent event) {
    ...
    LivingEntity entity = event.getEntity();
    if (entity.level() instanceof ServerLevel level && !level.getGameRules().get(GameRules.KEEP_INVENTORY)
        && entity instanceof Player player && !(entity instanceof FakePlayer)) {
  ...
  static void onPlayerClone(PlayerEvent.Clone event) {
    ...
    if (!(clone.level() instanceof ServerLevel level) || level.getGameRules().get(GameRules.KEEP_INVENTORY) || original.isSpectator()) {
      return;
    }
  ...
  private static int getSoulboundSlot(ItemStack stack) {
    ...
    return tag.getIntOr(SOULBOUND_SLOT, -1);
  }
